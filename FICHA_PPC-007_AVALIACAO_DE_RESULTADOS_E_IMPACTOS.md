---
id: FICHA-PPC-007
titulo: Ficha PPC-007 — Avaliação de Resultados e Impactos
versao: 0.1.0
status: rascunho
tipo: ferramenta
idioma: pt-BR
data_criacao: null
data_versao: 2026-07-18
responsaveis:
  - Projeto Pragmatismo Cívico
aprovadores: []
depende_de:
  - PPC-007
  - PPC-006
  - MODELO-TDM-001
  - PPC-META-001
  - ARQ-001
produz_entrada_para: []
relaciona_se_com:
  - CICLO-PC-001
  - PPC-008
  - INDICADORES
substitui: []
substituido_por: null
compatibilidade: inicial
proxima_revisao: null
documento_associado: PPC-007
versao_documento_associado: 0.1.0
---

# Ficha PPC-007 — Avaliação de Resultados e Impactos

Use esta ficha para planejar e registrar uma avaliação proporcional à importância, ao risco e à maturidade da intervenção.

## 1. Identificação

- Política, programa ou projeto:
- Unidade demandante:
- Equipe avaliadora:
- Período avaliado:
- Data de conclusão prevista:
- Grau de independência da avaliação:

## 2. Finalidade da avaliação

- decisão que a avaliação deve informar:
- público principal:
- uso esperado dos resultados:

## 3. Perguntas avaliativas

| Pergunta | Dimensão | Critério de resposta | Evidência necessária |
|---|---|---|---|
|  | processo, resultado, impacto, equidade ou economia |  |  |

## 4. Teoria da mudança

- resultados esperados:
- mecanismos causais:
- hipóteses críticas:
- fatores externos relevantes:

## 5. Desenho metodológico

- abordagem qualitativa:
- abordagem quantitativa:
- estratégia de comparação ou atribuição:
- unidade de análise:
- período de referência:
- limitações previstas:

## 6. Dados e evidências

| Evidência | Fonte | Qualidade | Cobertura | Limitação |
|---|---|---|---|---|
|  |  |  |  |  |

## 7. Resultados

| Pergunta | Achado | Força da evidência | Interpretação |
|---|---|---|---|
|  |  |  |  |

## 8. Efeitos distributivos e adversos

- grupos mais beneficiados:
- grupos menos beneficiados:
- impactos inesperados:
- efeitos adversos:
- riscos éticos ou de direitos:

## 9. Análise econômica

- custos observados:
- benefícios observados:
- custo por resultado relevante:
- alternativas de comparação:
- incertezas:

## 10. Conclusões e recomendações

| Recomendação | Evidência de suporte | Prioridade | Responsável | Prazo |
|---|---|---|---|---|
|  |  |  |  |  |

## 11. Resposta institucional

- recomendações aceitas:
- recomendações rejeitadas e justificativa:
- plano de ação:
- data de revisão:

## Checklist de qualidade

- [ ] finalidade e perguntas avaliativas explícitas;
- [ ] método adequado às perguntas;
- [ ] limitações declaradas;
- [ ] resultados separados de interpretações;
- [ ] equidade e efeitos adversos examinados;
- [ ] força da evidência indicada;
- [ ] recomendações rastreáveis aos achados;
- [ ] resposta institucional prevista.

## Histórico de alterações da ferramenta

| Versão | Data | Tipo | Alteração | Responsável |
|---|---|---|---|---|
| 0.1.0 | 2026-07-18 | inicial | Migração documental para o PPC-META-001, com vínculo à versão 0.1.0 do PPC-007 e preservação dos campos operacionais | Projeto Pragmatismo Cívico |
